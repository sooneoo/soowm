...
xwaylandready(struct wl_listener *listener, void *data)
{
	...
	if ((xcursor = wlr_xcursor_manager_get_xcursor(cursor_mgr, "default", 1))) {
        ...
        struct wlr_buffer *buf = wlr_xcursor_image_get_buffer(image); /* případně vytvoření bufferu z pixelů */

        wlr_xwayland_set_cursor(xwayland, buf, image->hotspot_x, image->hotspot_y);
    }
}
#endif
